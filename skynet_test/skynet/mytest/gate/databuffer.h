...
struct databuffer {
	int header;
	int offset;	//how much size the message at the tail have been read 
...
static inline void
_return_message(struct databuffer *db, struct messagepool *mp) {	//将databuffer 中的head向前移动一位，将释放了的message 连接到 messagepool中的freelist头部
...
static void
databuffer_read(struct databuffer *db, struct messagepool *mp, void * buffer, int sz) {	//从databuffer中读取sz大小字节到buffer，并回收dadabuffer中读完的message到pool中
	assert(db->size >= sz);
	db->size -= sz;
	for (;;) {
...
static void
databuffer_push(struct databuffer *db, struct messagepool *mp, void *data, int sz) {	//从messagepool 的freelist中取出一个message 并赋值，将其放入databuffer中
	struct message * m;
	if (mp->freelist) {
		m = mp->freelist;
		mp->freelist = m->next;
