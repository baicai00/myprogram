...
if not data then
    if not string.find(err, "timeout", 1, true) then --find函数不了解???
...
if typ == "close" then
    ...
    local byte, err = wb:send_close(1000, "enough, enough!")--???
    if not bytes then
...
elseif typ == "pong" then
    -- just discard th incoming pong frame
else
